...
namespace Nirvana.Models.TaskBar
{
...
#pragma warning disable CS0693 // Параметр типа имеет то же имя, что и параметр, указанный во внешнем типе
    ...
    public class CloseWindowCommand : CommandBase<CloseWindowCommand>
    {
        ...
        {
            GetTaskbarWindow((parameter as HeadViewModel).Tb).Close();
            CommandManager.InvalidateRequerySuggested();
            
        }
        public override bool CanExecute(object parameter)
        {
            var taskBar = parameter as HeadViewModel;
            Window win = null;
            if (taskBar != null)
                win = GetTaskbarWindow(taskBar.Tb);
            return win != null;
        }
    }
    ...
    public class HideSampleWindowCommand : CommandBase<HideSampleWindowCommand>
    {
        public override void Execute(object parameter)
        {
            ...
            GetTaskbarWindow((parameter as HeadViewModel).Tb).Hide();
            CommandManager.InvalidateRequerySuggested();
        }
    ...
    public class ShowSampleWindowCommand : CommandBase<ShowSampleWindowCommand>
    {
        public override void Execute(object parameter)
        {
            GetTaskbarWindow((parameter as HeadViewModel).Tb).Show();
            CommandManager.InvalidateRequerySuggested();
        }
